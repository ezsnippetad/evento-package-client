...
import {
  discountId,
  getSelectServiceId,
} from "../../../Pages/Dashboard/Event/Discount/discountSlice";
import { useIntl } from "react-intl";

const EventPopUpDiscountOnEquipmentOrItem = ({
  handleClose,
  eventId,
  allDiscount,
  setAllDiscount,
    ...
    try {
      ...
      if (eventDetails?.data.IsSuccess) {
        try {
          ...
          res?.data?.Data?.discounts
            .filter(
              (type) => type.discounttype === "discount_on_equipment_or_item",
            )
            .map((e, i) => {
        ...
        <div className="table-cell align-middle">
          <div className="popin max-w-2xl w-full mx-auto max-h-[calc(100vh-55px)] overflow-y-auto lg:px-9">
            <div className="bg-brightGray p-7 space-y-7">
              <h2 className="h1 w-full max-w-[60%] max-[768px]:max-w-[80%] text-center mx-auto">
                {" "}
