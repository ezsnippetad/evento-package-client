...
const EventPopUpAddEquipment = ({ handleClose, data, setReload, edit }) => {
  ...
  useEffect(() => {
    if (data && edit) {
      ...
      setImageList(data.photos[0])
      setVideo(data.videos[0]?.url);
      setVideoList(data.videos[0])
    }
  ...
  const addServices = async () => {
    ...
    if (name.trim() === "" || price.trim() === "" || quantity.trim() === "") {
      toast.warn(
        `${intl.formatMessage({ id: "PLEASE FILL ALL THE REQIRED FIELDS." })}`,
      );
      return;
    }
    ...
    if (!onlyDigits.test(quantity.trim())) {
      toast.warn(
        `${intl.formatMessage({ id: "PLEASE ENTER VALID QUNATITY." })}`,
      );
      return;
    }
    ...
    try {
      ...
      payload.photos.push({ url: imageList?.url })
      payload.videos.push({ url: videoList?.url })
        ...
        <div className="popin max-w-2xl w-full mx-auto max-h-[calc(100vh-55px)] overflow-y-auto lg:px-9">
          <div className="bg-brightGray p-12 max-[640px]:px-5">
            <div className="flex justify-between items-center">
              <h1 className="h1">
                {intl.formatMessage({ id: "ADD EQUIPMENT" })}
              </h1>
            ...
            <form className="space-y-5 py-8">
              ...
              <div className="w-full">
                ...
                <label
                  htmlFor=""
                  className="flex items-center w-full bg-white p-2 px-3.5 rounded-md max-[640px]:flex-col max-[640px]:pl-1"
                >
              ...
              <div className="upload-holder">
                ...
                <label htmlfor="upload" className="upload upload-popup">
                  ...
                  <span className="input-titel mt-1">
                    ...
                    {intl.formatMessage({ id: "CHOOSE VIDEO" })}
                  </span>
                </label>
                {error2 ? (
                  <span
                  ...
                  <span className="mt-1" style={{ fontSize: "14px" }}>
                    {video?.name ||
                      (video && (
                        <a target="blank" href={s3Url + "/" + video}>
                    ...
                    <div className="upload-box cursor-pointer" >
                      <div className="rounded relative overflow-hidden h-full">
                        <video className="h-full">
                          <source
                            src={s3Url + "/" + videoList?.url}
                          // alt={"upload-" + index}
                          />
                        </video>
                        <button
                        // onClick={() => removeVideoClick(vid)}
                        >
                          {intl.formatMessage({ id: "REMOVE" })}
                        </button>
                      </div>
