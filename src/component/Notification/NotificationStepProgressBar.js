...
const NotificationStepProgressBar = () => {
  const intl = useIntl();
  const NotificationHYP = [
    `${intl.formatMessage({ id: "SELECT BUSINESS" })}`,
    `${intl.formatMessage({ id: "USER" })}`,
    `${intl.formatMessage({ id: "PUBLISH DATE & TIME" })}`,
    `${intl.formatMessage({ id: "BILL DETAILS" })}`,
    `${intl.formatMessage({ id: "PAYMENT" })}`,
  ];
  const nCount = useSelector(
    (state) => state?.notificationstepPogressCount?.nCount,
  );
  return (
      ...
      <ul className="flex justify-between step-progress-holder">
        {NotificationHYP.map((element, index) => (
          <li className={nCount >= index + 1 ? "active" : ""} key={index}>
            <div>
              <span className={nCount >= index + 1 ? "active" : ""}>
                {index + 1}
              </span>
            </div>
            <h3>{element}</h3>
