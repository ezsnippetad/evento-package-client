            ...
            <>
              {invoice.docs?.length ? (
                invoice.docs?.map((e) => (
                  <div
                    key={e?._id}
                    className="w-full bg-white flex items-center rounded-lg cursor-pointer"
                  >
                    <div
                      className="flex w-full justify-between items-center p-6 pr-0"
                      onClick={() => {
                        localStorage.setItem("invoiceId", e?._id);
                        navigate("invoicedetials");
                      }}
                    >
                      <div className="flex items-center space-x-6">
                        <p className="h2">{e.invoice_no}</p>
                        <p className="h2 font-medium text-quicksilver">
                          {e?.name}
                        </p>
                        <p className="h2 font-medium text-quicksilver">
                          {e?.userid?.name}
                ...
                <div className="bg-white rounded-md text-center p-9 space-y-2 ng-star-inserted">
                  <h1 className="w-full">You have no Invoice yet.</h1>
                </div>
              )}
            </>
          )}
          {!loading &&
            ...
            (invoice?.totalPages > 0 ? (
              ...
              <h1 style={{ margin: "100px 0" }}>
                {intl.formatMessage({ id: "NO INVOICE FOUND" })}
              </h1>
            ))}
        </div>
      </div>
