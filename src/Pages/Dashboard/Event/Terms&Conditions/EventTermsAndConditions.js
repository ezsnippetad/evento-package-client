...
const EventTermsAndConditions = () => {
  ...
  const saveData = async () => {
    if (!isCheck) {
      toast.warn(
        `${intl.formatMessage({
          id: "PLEACE ACCEPT THE TERMS AND CONDITION.",
        })}`,
      );
      return;
    }
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-end">
              ...
              <span className="input-titel text-base ml-4">
                {intl.formatMessage({ id: "ACCEPT YOUR" })}{" "}
                <button
                  className="text-spiroDiscoBall"
                  onClick={() => setIsTermsAndConditionPopUpOpen(true)}
                >
                  {intl.formatMessage({ id: "TERMS AND CONDITIONS" })}
                </button>
              </span>
            </div>
            <button className="btn-primary w-full" onClick={() => saveData()}>
